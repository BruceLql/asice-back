 ...
 * https://www.cnblogs.com/li-ran/p/6824755.html
 * @Description 
 */
public class ComputerMonitorUtil {
	...
	public static String diskzh="";//磁盘总和
	public static String diskky="";//磁盘可用
	public static String memky="";//内存可用
	/**
	...
	/**
	 * 功能：Linux 和 Window 内存使用率 <br>
	 * 内存使用率：[1 -  剩余的物理内存/(总的物理内存+虚拟内存) ] * 100 <br>
	 * */
	public static double getMemUsage() {
		if (osName.toLowerCase().contains("windows")
			...
			try {
				...
				memky=getDataSize(String.valueOf(freePhysicalMemorySize));
			...
			try {
				...
				memky=getDataSizeBy(String.valueOf(memTotal-(memused - buffers - cached)),"k");
				BigDecimal b1 = new BigDecimal(usage);
				double memoryUsage = b1.setScale(2, BigDecimal.ROUND_HALF_UP)
						.doubleValue();
				return memoryUsage;
	...
	/**
	 * Window 和Linux 得到磁盘的使用率 <br>
	 * 磁盘总使用率：[1 -  磁盘空余大小/磁盘总大小 ] * 100 <br>
	 * @return
	 * @throws Exception
	 */
	public static double getDiskUsage() throws Exception {
		...
		if (osName.toLowerCase().contains("windows")
			...
			diskky=getDataSize(String.valueOf(allFree));
			diskzh=getDataSize(String.valueOf(allTotal));
			...
			// 保留2位小数
			diskky=getDataSizeBy(String.valueOf(totalHD-usedHD),"m");
			diskzh=getDataSizeBy(String.valueOf(totalHD),"m");
	...
	private static String getDataSize(String size) {
		String w="";
		if(size!=null){
			try {
	...
	private static String getDataSizeBy(String size,String danwei) {
		if(size!=null){
			String[] dan={"Byte","KB","MB","GB","TB","PB"};
			int a=0;
			if(danwei.equalsIgnoreCase("byte")||danwei.equalsIgnoreCase("b")){
				a=0;
			}else if(danwei.equalsIgnoreCase("kb")||danwei.equalsIgnoreCase("k")){
				a=1;
			}else if(danwei.equalsIgnoreCase("mb")||danwei.equalsIgnoreCase("m")){
				a=2;
			}else if(danwei.equalsIgnoreCase("gb")||danwei.equalsIgnoreCase("g")){
				a=3;
			}else if(danwei.equalsIgnoreCase("tb")||danwei.equalsIgnoreCase("t")){
				a=4;
			}
			try {
				double d=Double.parseDouble(size);
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("cpuUsage(cpu使用率):" + cpuUsage);
		System.out.println("memUsage(内存使用率):" + memUsage);
		System.out.println("diskUsage(磁盘总使用率):" + diskUsage);
