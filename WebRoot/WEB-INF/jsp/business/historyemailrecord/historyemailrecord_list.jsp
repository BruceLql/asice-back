    ...
    <div id="page-content" class="clearfix">
        ...
        <div class="row-fluid">
            ...
            <div class="row-fluid">
                ...
                <!-- 检索  -->
                <form action="historyemailrecord/list.do" method="post" name="Form" id="Form">
                    <table>
                        <tr>
                            <td>
                    ...
                    <table id="table_report" class="table table-striped table-bordered table-hover">
                        ...
                        <tr>
                            ...
                            <th class="center">发件人邮箱账户</th>
                            <th class="center">收件人邮箱账户</th>
                            <th class="center">邮件标题</th>
                            <%--						<th class="center">邮件正文</th>--%>
                            <th class="center">邮件发送时间</th>
                            <%--						<th class="center">邮件发送状态(0：发送失败；1:发送成功 2：其他状态)</th>--%>
<%--                            <th class="center">创建时间</th>--%>
                            <%--						<th class="center">状态(0:正常 -1：失效)</th>--%>
                            <%--						<th class="center">备用</th>--%>
                                        ...
                                        <tr>
                                            <td class='center' style="width: 30px;">
                                                <label><input type='checkbox' name='ids'
                                                              value="${var.HISTORYEMAILRECORD_ID}"/><span
...
<%--                                            <td>${var.CREATETIME}</td>--%>
                                                <%--										<td>${var.STATUS}</td>--%>
                                                <%--										<td>${var.HISTORY_001}</td>--%>
                                                <%--										<td>${var.HISTORY_002}</td>--%>
                                            <td style="width: 30px;" class="center">
                                                <div class='hidden-phone visible-desktop btn-group'>
													...
													<c:if test="${QX.edit == 1 }">
                                                        <button class="" style="background:#b0d877; " title="邮件内容" onclick="showEmailContent('${var.HISTORYEMAILRECORD_ID}','${var.EMAIL_TITLE}');">
															查看详情
														</button>
													</c:if>
                                                        ...
                                                        <ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
                                                            <c:if test="${QX.del == 1 }">
                                                                <li><a style="cursor:pointer;" title="删除"
                                                                       onclick="del('${var.HISTORYEMAILRECORD_ID}');"
...
<script type="text/javascript">
    ...
    function del(Id) {
        bootbox.confirm("确定要删除吗?", function (result) {
            if (result) {
                top.jzts();
                var url = "<%=basePath%>historyemailrecord/delete.do?HISTORYEMAILRECORD_ID=" + Id + "&tm=" + new Date().getTime();
    ...
    //查看邮件内容
    function showEmailContent(Id, title) {
        top.jzts();
        var diag = new top.Dialog();
        diag.Drag = true;
        diag.Title = title;
        diag.URL = '<%=basePath%>historyemailrecord/showEmailContent.do?HISTORYEMAILRECORD_ID=' + Id;
        diag.Width = 1200;
        diag.Height = 500;
...
<script type="text/javascript">
            ...
            if (result) {
                ...
                if (str == '') {
                    ...
                    if (msg == '确定要删除选中的数据吗?') {
                        ...
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>historyemailrecord/deleteAll.do?tm=' + new Date().getTime(),
                            data: {DATA_IDS: str},
                            dataType: 'json',
    ...
    //导出excel
    function toExcel() {
        window.location.href = '<%=basePath%>historyemailrecord/excel.do';
    }
</script>
