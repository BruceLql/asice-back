...
%>
<!DOCTYPE html>
<html lang="en" id="1530144241072-8335">
...
<head>
	...
	<!-- 确认窗口 -->
<!-- 自定义皮肤 -->
<link rel="stylesheet" href="static/css/diytheme-a.css">
		...
		<div class="row-fluid">
			...
			<form action="<%=basePath%>/dictionaries.do" method="post" name="userForm" id="userForm">
				...
				<table>
					<tr>
						<!-- <td>
							<font color="#808080">检索：</font>
						</td> -->
						<td>
							<span class="input-icon">
									
								<input type="text" name="NAME" value="${pd.NAME }" placeholder="这里输入名称" style="width:130px;" />
						...
						<c:if test="${pd.PARENT_ID != '0'}">
							<c:choose>
								<c:when test="${not empty varsList}">
									<td style="vertical-align:top;">
										<a href="<%=basePath%>/dictionaries.do?PARENT_ID=0" class="btn btn-mini btn-purple" title="查看">顶级
											<i class="icon-arrow-right  icon-on-right"></i>
										</a>
									</td>
									<c:forEach items="${varsList}" var="var" varStatus="vsd">
										<td style="vertical-align:top;">
											<a href="<%=basePath%>/dictionaries.do?PARENT_ID=${var.ZD_ID }" class="btn btn-mini btn-purple" title="查看">${var.NAME }
												<i class="icon-arrow-right  icon-on-right"></i>
											</a>
										</td>
									</c:forEach>
						...
						<c:when test="${not empty varList}">
							<c:forEach items="${varList}" var="var" varStatus="vs">
								<tr>
									...
									<td class='center'>
										<a href="<%=basePath%>/dictionaries.do?PARENT_ID=${var.ZD_ID }" title="查看下级">
											<i class="icon-arrow-right  icon-on-right"></i>&nbsp;${var.NAME }</a>
									</td>
									...
									<td style="width: 68px;">
										<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${var.ZD_ID }')">
											<i class='icon-edit'></i>
										</a>
										<a class='btn btn-mini btn-danger' title="删除" onclick="del('${var.ZD_ID }')">
