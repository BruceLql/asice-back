...
%>
<!DOCTYPE html>
<html lang="en" id="1530144764960-3416">
	<head>
		<base href="<%=basePath%>">
		<meta charset="utf-8" />
		<title></title>
		...
		$.ajax({
			...
			success: function(data){
				 if("success" == data.result){
					...
					setTimeout("$('#loginname').val('此登录名称已存在!')",500);
				 }
			}
		});
	}
...
<body>
	<form action="user/${msg }.do" name="userForm" id="userForm" method="post">
			...
			<table>
				<c:if test="${fx != 'head'}">
					<c:if test="${pd.ROLE_ID != '1'}">	
						...
						<%--<tr>
							<td>
								<select class="chzn-select" name="gs_id" id="gs_id" data-placeholder="请选择客户公司" >
									<option value=""></option>
									<c:forEach items="${list_gs}" var="v" >
										<option value="${v.mrc_uuid }" <c:if test="${v.mrc_uuid== pd.gs_id }">selected</c:if>>${v.mrc_name }</option>
									</c:forEach>
								</select>
							</td>
						</tr>
						<tr>
							<td id="td2">
								<select class="chzn-select" name="bm_id" id="bm_id" data-placeholder="请选择客户部门" >
									<option value=""></option>
									<c:forEach items="${list_bm}" var="v" >
										<option value="${v.mrc_uuid }" <c:if test="${v.mrc_uuid== pd.bm_id }">selected</c:if>>${v.mrc_name }</option>
									</c:forEach>
								</select>
							</td>	
						</tr>--%>
				...
				<tr>
					<td><input type="text" name="USERNAME" id="loginname" value="${pd.USERNAME }" maxlength="32" placeholder="这里输入登录名称" title="登录名称"/></td>
				</tr>
				<%--<tr>
		...
		$(function() {
			/* $("#RISK_A_EXT_SYS_CP_ID").change(function(){
				var cp_id =$(this).val();
				$("#td1").empty();
				$.ajax({
					url:'company/findCompanyDep',
					data:{"cp_id":cp_id},
					dataType:"json",
					type:"post",
					success:function(data){
						console.log((data));
						var $select=$("<select name='RISK_A_EXT_SYS_CP_DEPID'>");
						
						$.each(data.list,function(index,item){
							var $option=$("<option value='"+item.RISK_A_EXT_SYS_CP_DEPID+"'>"+item.RISK_A_EXT_SYS_CP_DEP+"</option>");
			...
			$("#gs_id").change(function(){
				...
				//$("#bm_id").empty();
				$.ajax({
					url:'corporatesector/findBMbyGS.do',
					data:{"mrc_uuid":cp_id},
					dataType:"json",
					type:"post",
					success:function(data){
						console.log((data));
						var $select=$("<select name='bm_id'>");
						$.each(data.list,function(index,item){
							var $option=$("<option value='"+item.mrc_uuid+"' >"+item.mrc_name+"</option>");
							$select.append($option);
						});
						$("#td2").append($select);
					},
